...
def download_dynamic_world(lat: float, lon: float, bands: list, data_dir: str):
    os.makedirs(data_dir, exist_ok=True)

    point = ee.Geometry.Point([lon, lat])
    dw_collection = ee.ImageCollection("GOOGLE/DYNAMICWORLD/V1").select(bands).getRegion(point, scale=10)

    data = dw_collection.getInfo()
    df = ee_array_to_df(data, list_of_bands=bands)
...
if __name__ == "__main__":
    ...
    download_dynamic_world(args.lat, args.lon, DW_BANDS, args.output_dir)